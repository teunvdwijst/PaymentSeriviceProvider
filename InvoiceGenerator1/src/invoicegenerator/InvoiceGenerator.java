...
public class InvoiceGenerator extends Application {
    ...
    final AppGateway gateway = new AppGateway("Client1ToPSP", "PSPToClient1");
    ...
    private final String generatedBy = "Client1";

    private InvoiceRequest SendInvoice() {
        InvoiceRequest request = new InvoiceRequest(Double.parseDouble(subject.getText()), "EUR", cbPaymentMethod.getValue().toString().toLowerCase(), generatedBy);
        return request;
    }

    @Override
    public void start(Stage primaryStage) {
        primaryStage.setTitle("InvoiceGenerator Client1");
